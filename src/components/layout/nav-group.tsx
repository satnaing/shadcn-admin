...
function checkIsActive(path: string, item: NavItem, mainNav = false) {
  // main heading nav is active if any child nav is active
  if (item.items && item?.items?.filter((i) => i.url?.startsWith(path)).length) return true

  // for non main ones, if theres no url return
  if (!item.url) return

  return (
    path === item.url || // /endpint?search=param
    path.split('?')[0] === item.url || // endpoint
    path.startsWith(item.url || '') || // endpoint/nested
    // if child nav is active
    (mainNav && path.split('/')[1] !== '' && path.split('/')[1] === item?.url?.split('/')[1])
  )
}

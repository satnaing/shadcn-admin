...
export const executionsColumns: ColumnDef<Execution>[] = [
  {
    ...
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title='Playbook' />
    ),
    cell: ({ row, table }) => {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const summary = row.getValue('result')?.shortSummary as string | null
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <span className='max-w-[300px] cursor-default'>
                {truncated}
              </span>
            </TooltipTrigger>
            {summary.length > 80 && (
